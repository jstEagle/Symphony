  ...
  return (
    ...
    >
      {!loaded && !error && (
        ...
        >
          <AgentLoader kind="square" size={28} label="Loading image" />
        </div>
      )}
      {error ? (
        <div
        ...
        >
          <ImageBroken className="text-muted-foreground size-8" />
        </div>
      ) : (
        <img
          ref={imgRef}
      ...
      {isMounted &&
        ...
        createPortal(
          <div
            ...
            role="dialog"
            aria-modal="true"
            className="aui-image-zoom-overlay fade-in animate-in fixed inset-0 z-50 flex items-center justify-center bg-[var(--overlay)] duration-200"
            onClick={handleClose}
...
function ImageGenerating({ className }: { className?: string }) {
  return (
    ...
    >
      <AgentLoader kind="circular" size={28} label="Generating image" />
